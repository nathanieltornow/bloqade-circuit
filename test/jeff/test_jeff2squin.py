...
def _handed_back(mt: ir.Method) -> tuple[int | None, ...]:
    references = WireReferenceAnalysis(jeff.kernel)
    references.run(mt)
    return handed_back(mt, references.results[mt])
...
def test_handed_back_through_a_switch_and_a_subset():
    ...
    assert _handed_back(callee) == (None, 1)
...
def test_inputs_handed_back_in_another_order_are_refused():
    block, (q0, q1) = entry(jeff.WireType, jeff.WireType)
    mt = method(block, (q1, q0), types.Generic(tuple, jeff.WireType, jeff.WireType))
    with rejected("hands its inputs back in another order"):
        JeffToSquin().emit(mt)
...
def test_a_register_with_a_replaced_slot_is_a_new_register():
    ...
    assert _handed_back(callee) == (None,)
...
@pytest.mark.xfail(
    reason="kirin types a tuple read at index 0 as the union of all members",
    strict=True,
)
def test_a_qubit_at_tuple_position_zero_beside_a_classical_output():
    """A qubit at position 0 beside a classical output must stay a qubit
    through the call's tuple read."""
    block, _ = entry()
    fresh = add(block, stmts.Alloc()).result
...
def test_out_of_order_hand_back_in_a_callee_is_refused_by_validation():
    ...
    for wire in call.results:
        add(outer, stmts.Free(wire))
    with rejected("hands its inputs back in another order"):
        JeffToSquin().emit(method(outer, None, types.NoneType))
...
def test_registers_that_swap_qubits_are_not_handed_back():
    ...
    assert _handed_back(mt) == (None, None)
...
def test_the_validation_suite_runs_the_conversion_validation_on_its_own():
    ...
    assert any("hands its inputs back in another order" in m for m in messages)
